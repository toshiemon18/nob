...
RSpec.describe Nob::Templates::Operators do
  describe ".build" do
    ...
    it "raises UndefinedVariable for unknown name" do
      expect { described_class.build(name: "foo", fmt: nil) }.to(
        raise_error(
    ...
    it "propagates UndefinedVariable from operator constructor (e.g., title with fmt)" do
      expect { described_class.build(name: "title", fmt: "x") }.to(
        raise_error(
          Nob::Templates::UndefinedVariable,
          /title does not accept format/
        )
      )
    end

    it "rejects a name with mixed case (Title) as UndefinedVariable" do
      expect { described_class.build(name: "Title", fmt: nil) }.to(
        raise_error(
          Nob::Templates::UndefinedVariable,
